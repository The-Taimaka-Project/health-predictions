...
Original file is located at
    https://colab.research.google.com/drive/1Szx-fy3AhHjCBwVnuB8pD5H6smD5NaY1
"""
...
def gbm_shap(features,N_FEATURES,X_train_transformed,X_test_transformed,X_test_transformed_top,y_train,y_test,cutoff=0.5):
  import shap
...
def drop_recent_columns(detn,use_cache=True,months=15):
  ...
  if use_cache:
    ...
    detn.drop(columns=weekly_raw_columns_to_delete.intersection(set(detn.columns)),inplace=True)
    ...
    for col in raw.columns:
      series1 = raw[col].notnull().groupby(raw['todate'].dt.to_period('M')).sum()/raw['todate'].groupby(raw['todate'].dt.to_period('M')).count()
    ...
    for col in admit.columns:
      series1 = admit[col].notnull().groupby(admit['calcdate'].dt.to_period('M')).sum()/admit['calcdate'].groupby(admit['calcdate'].dt.to_period('M')).count()
      populated_months = (series1 > 0).sum()
      admit_columns.loc[col,'populated_months'] = populated_months
    recent_admit_columns = admit_columns[admit_columns['populated_months'] < months].index.to_list()
    print('recent_admit_columns',recent_admit_columns)

    weekly.dropna(axis=1, how='all', inplace=True)
    weekly_columns = pd.DataFrame()
    for col in weekly.columns:
      series1 = weekly[col].notnull().groupby(weekly['calcdate'].dt.to_period('M')).sum()/weekly['calcdate'].groupby(weekly['calcdate'].dt.to_period('M')).count()
      populated_months = (series1 > 0).sum()
      weekly_columns.loc[col,'populated_months'] = populated_months
    recent_weekly_columns = weekly_columns[weekly_columns['populated_months'] < months].index.to_list()
    ...
    for col in weekly_raw.columns:
      series1 = weekly_raw[col].notnull().groupby(weekly_raw['todate'].dt.to_period('M')).sum()/weekly_raw['todate'].groupby(weekly_raw['todate'].dt.to_period('M')).count()
      populated_months = (series1 > 0).sum()
      weekly_raw_columns.loc[col,'populated_months'] = populated_months
    recent_weekly_raw_columns = weekly_raw_columns[weekly_raw_columns['populated_months'] < months].index.to_list()
