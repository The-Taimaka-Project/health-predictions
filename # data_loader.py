...
DB_USER=jostrowski
DB_PASSWORD=bobbing-tidy-untagged
ODK_USERNAME=jennifer@taimaka.org
ODK_PASSWORD=deutsch6TH
...
def extend_current(current_df, admit_raw_df):
    new_pids = [
        "23-9999", "23-9998", "23-9997", "24-9999", "24-9991", "24-9998", "24-9990",
    ...
    add_current = pd.DataFrame({
        "pid": new_pids,
        "phone": [
            "701-729-1416", None, "903-395-7213", "902-236-5364", None, "803-769-9436", None,
            None, "806-380-3885", "708-720-7492", "706-611-7182", "903-754-3375", "802-670-5817", None, "901-131-0883",
        ...
        "phoneowner": [None]*17,
        "site": [
            "sangaru", "kuri", "jalingo", "sangaru", "kuri", "kurjale", None,
            "sangaru", "sangaru", "sangaru", "sangaru", "sangaru", "sangaru", "sangaru", "jalingo",
            "kuri", "sangaru"
        ],
        "status": [
            "dead", "dead", "excluded", "dead", "dead", "default", "excluded",
            "default", "default", "default", "default", "default", "default", "default", "default",
            "dead", "default"
        ],
        "status_detail": [
            None, None, "defect", None, None, "final", "other",
            "final", "final", "final", "final", "final", "final", "final", "final",
            None, "returnable"
        ],
        "movement_detail": [None]*17,
        "status_date": pd.to_datetime([
            "2023-07-05", "2023-07-04", "2023-07-26", "2024-02-02", "2024-10-23", "2024-03-04", "2024-03-13",
    ...
    add_current["age"] = (
        add_current["em_age"].fillna(0).astype(int)
        + ((today - add_current["todate"]).dt.days // 30)
    )

    return pd.concat([current_df, add_current.drop(columns=["em_age", "todate"])], ignore_index=True)

def extend_deaths(deaths_df):
    add_deaths = pd.DataFrame({
        "pid": ["23-9999", "23-9998", "24-9999", "24-9991", "24-9989"],
        "dod": pd.to_datetime(["2023-07-05", "2023-07-04", "2024-02-02", "2024-10-23", "2024-12-05"]),
        "notes": [
...
def merge_notes(current_df, exclusions_df, deaths_df, new_pids):
    df = current_df.merge(
        exclusions_df[["pid", "notes"]],
        on="pid", how="left"
    ).rename(columns={"notes": "exclusion_notes"})
    ...
    df = df.merge(
        deaths_df[["pid", "notes"]],
        on="pid", how="left"
    ).rename(columns={"notes": "death_notes"})
...
def build_admit_processed(admit_raw_df, admit_df, new_pids, imci_pids):
    add_admit_processed = admit_raw_df[
        admit_raw_df["pid"].isin(new_pids)
    ].copy()
    ...
    add_admit_processed["md_starttime"] = pd.to_datetime(add_admit_processed["start_time"], errors="coerce")
    add_admit_processed["b_referred_emergency"] = add_admit_processed["imci_emergency_otp"] == "true"
    add_admit_processed["b_presented_emergency"] = add_admit_processed["imci_emergency_itp"] == "true"
...
# -- Load 2023 --
itp_2023 = load_google_sheet("https://docs.google.com/spreadsheets/d/1Pb_bGGaHRIyzwHhIBzebFqQhd-6HQttWKDeTvRR58-o/edit#gid=0")

# -- Clean 2023 --
itp_2023_clean = (
    itp_2023[itp_2023["Facility"].notna()]
    .assign(
        ...
        outcome_date=pd.to_datetime(itp_2023["Outcome date"].combine_first(itp_2023["LOS"]), errors="coerce"),
        pid=itp_2023["Reg. No"],
        ...
        case_notes=itp_2023[["Mgt checked in folder", "notes_2", "notes_3"]].astype(str).agg(", ".join, axis=1)
    )
)
...
itp_2023_clean.loc[itp_2023_clean["pid"] == "-", "pid"] = itp_2023_clean["Name"].map({
    "Mohmmed Lawadu": "23-9998",
    "Ali Adamu": "23-9999",
    "Laure Adamu": "23-9997"
})

# Drop unnecessary columns
itp_2023_clean = itp_2023_clean.drop(columns=[
    "Reg. No", "Week #", "Name", "LOS", "Outcome date", "Referred with Reg. No",
...
def clean_itp_roster(df, year):
    ...
    return df.drop(columns=[
        "Pid", "Age (months)", "MUAC (cm)", "Date of Admission", 
...
itp_2024 = load_google_sheet("https://docs.google.com/spreadsheets/d/1kWdcIL7ajRxHfmtS7R3BNkUzUgEac-XCWlzM4xNlkuA/edit?gid=0")
itp_2024_clean = clean_itp_roster(itp_2024, year=2024)

itp_2025 = load_google_sheet("https://docs.google.com/spreadsheets/d/11LqjmNJeHNLirCaYSijpJms0Rp1NZbJDKZ0WxtSAqkk/edit?gid=0")
...
def clean_itp_final(df):
    return (
        df.assign(
            otp=df["otp"].replace({
                "Kurjelli": "kurjale", "PHC Kurjele": "kurjale",
                "Ashaka": "jalingo", "PHC Ashaka": "jalingo",
                "Kuri": "kuri", "PHC Kuri": "kuri",
                "Sangaru": "sangaru", "PHC Sangaru": "sangaru"
            }),
            itp=df["itp"].replace({
                "FTH": "fth", "SSHG": "ssh",
                "GH Deba": "gh_deba", "GH Bajoga": "gh_bajoga"
            }),
            ...
            case_notes=df.apply(
                lambda row: "Treated as out patient" if row["los_days"] == "Treated as out patient" else row["case_notes"],
                axis=1
            )
        )
...
def clean_admit_raw(admit_raw_df):
    ...
    filtered["b_has_phone_number"] = (
        filtered["phone"].notna() | filtered["em_phone"].notna()
    )

    # Step 3: Drop columns
    cols_to_drop = [
        "phone", "em_phone", "cg_name", "hoh_name", "homedesc", "hoh_wkname",
        "tradleader", "pp_phone", "pp_hoh", "em_pp_phone", "em_cg_name",
        "em_homedesc", "name", "pull_prev_name", "pull_prev_cg_name",
        "twin_name", "cg_wkname", "instance_name"
    ]
...
def clean_relapse_raw(relapse_raw_df, relapse_pids, current_pids):
    df = relapse_raw_df[
        ...
        ((relapse_raw_df["set_final_consent"].isna()) | (relapse_raw_df["set_final_consent"] != "refused")) &
    ...
    pii_cols = [col for col in df.columns if col.startswith("pull_") or col.startswith("homedesc") or "phone" in col or "contact" in col or "address" in col or "cg" in col or "name" in col]
...
def clean_mh_raw(mh_raw_df, mh_pids, current_pids):
    ...
    # Drop PII columns
    pii_cols = [col for col in df.columns if "pull_" in col or "name" in col or "address" in col or "phone" in col or "cg" in col or "leader" in col or "hoh" in col]
    df = df.drop(columns=pii_cols, errors="ignore")
...
def finalize_datasets(current_df, admit_df, weekly_df, itp_roster_df, current_pids):
    ...
    admit_processed = admit_processed.drop(columns=[
        "name", "phone", "cg_name", "hoh_name", "homedesc", "hoh_wkname", "tradleader_name"
    ], errors="ignore")
    ...
    # Add sex from admit
    weekly_processed = weekly_processed.merge(
        admit_processed[["pid", "sex"]], on="pid", how="left"
    )
...
from pygrowup.references import who2006

# Initialize the z-score calculator with WHO 2006 growth standards
calc = Calculator(ref=who2006)


def prepare_zscore_inputs(df, age_column="age", weight_column="weight", height_column="finalhl", domhl_column="domhl", sex_column="sex"):
    df = df.copy()
    ...
    df["standing"] = np.select(
        [df[domhl_column] == "height", df[domhl_column] == "length"],
        [1, 2],
        default=3
    )
    ...
    df["sex_num"] = np.select(
        [df[sex_column] == "male", df[sex_column] == "female"],
        [1, 2],
        default=np.nan
    )
...
def add_zscores(df, weight_col="weight", height_col="finalhl", age_col="age_zscore", sex_col="sex_num", standing_col="standing"):
    df = df.copy()
    ...
    def safe_calc(row, index):
        try:
            if index == "wfh":
                return calc.zscore("wfh", sex=int(row[sex_col]),
                                    weight=float(row[weight_col]),
                                    height=float(row[height_col]),
                                    standing=int(row[standing_col]))
            elif index == "hfa":
                return calc.zscore("hfa", sex=int(row[sex_col]),
                                    height=float(row[height_col]),
                                    age_days=float(row[age_col]))
            elif index == "wfa":
                return calc.zscore("wfa", sex=int(row[sex_col]),
                                    weight=float(row[weight_col]),
                                    age_days=float(row[age_col]))
        except:
            return np.nan
...
def add_deficiency_flags(df):
    df = df.copy()
    df["b_wast"] = np.where((df["wfhz"] < -3) & (df["hfaz"] < -3), True, np.where(df[["wfhz", "hfaz"]].isnull().any(axis=1), np.nan, False))
    df["b_muac_waz"] = np.where((df["muac"] < 11.5) & (df["wfaz"] < -3), True, np.where(df[["muac", "wfaz"]].isnull().any(axis=1), np.nan, False))
    df["b_muac_wfh"] = np.where((df["muac"] < 11.5) & (df["wfhz"] < -3), True, np.where(df[["muac", "wfhz"]].isnull().any(axis=1), np.nan, False))
    return df
...
def add_breastfeeding_flags(df):
    ...
    df["bfeed_age"] = np.select([
        ...
        (df["b_curr_bfeed"] == 'true') & (df["age"] > 5) & (df["age"] < 24) & df["alt_foods_bfeed"].astype(str).str.contains("food", na=False),
        (df["age"] > 24),
        (df["b_curr_bfeed"].isnull())
    ], [True, True, np.nan, np.nan], default=False)
    ...
    df["bfeed_exc"] = np.select([
        ...
        (df["b_curr_bfeed"].isnull())
    ], [True, True, np.nan, np.nan, np.nan], default=False)

    df["bfeed_exc_food"] = np.select([
        (df["age"] < 6) & (df["b_curr_bfeed"] == 'true') & (df["alt_foods_bfeed"] == 'false'),
        (df["age"] < 6) & (df["b_curr_bfeed"] == 'true') & df["alt_foods_bfeed"].astype(str).str.contains("food", na=False),
        (df["age"] > 5) & (df["age_takefamily"] > 5),
        (df["age"] > 5) & (df["age_takefamily"].isnull()),
        (df["b_curr_bfeed"].isnull())
    ], [True, True, True, np.nan, np.nan], default=False)
...
# For admit_raw_2
admit_raw_2 = prepare_zscore_inputs(admit_raw_2, height_column="hl", domhl_column="direction_of_measure", sex_column="c_sex")
...
# For weekly_raw_2
weekly_raw_2 = prepare_zscore_inputs(weekly_raw_2, height_column="hl", domhl_column="direction_of_measure", sex_column="c_sex")
